...
{
“role”: “system”,
“content”: “Tu es CryptoMind, expert en cryptomonnaies. R?ponds en fran?ais, de fa?on courte et pr?cise. Mentionne toujours les risques. Pas de conseil financier certifi?.”
},
{
“role”: “user”,
“content”: prompt
...
?? <i>Pas un conseil financier</i>”””, chat_id)
...
send_telegram(”? Scan en cours, r?essaie dans 30s.”, chat_id)
return
...
msg = f”? <b>TOP {n} CRYPTOS</b>\n<i>{len(all_prices_cache)} cryptos analys?es</i>\n\n” + “\n\n”.join(lines)
...
if not coin:
send_telegram(f”? <b>{symbol}</b> non trouv?. V?rifie le symbole (ex: BTC, ETH, SOL).”, chat_id)
return
...
return
send_telegram(f”?? Analyse de l’adresse…\n<code>{address}</code>”, chat_id)
send_telegram(check_fraud(address), chat_id)

def handle_best_now(chat_id):
send_telegram(”? Recherche de la meilleure opportunit?…”, chat_id)
if not all_prices_cache:
send_telegram(”? Scan en cours, r?essaie dans 30s.”, chat_id)
return
...
if not all_prices_cache:
send_telegram(”? Donn?es non disponibles.”, chat_id)
return
...
msg = (f”? <b>MARCH? ? {datetime.now().strftime(’%H:%M’)}</b>\n”
...
f”? Mis ? jour: <b>{datetime.now().strftime(’%H:%M:%S’)}</b>”, chat_id)
...
f”Donne : signal ACHETER/VENDRE/ATTENDRE, prix d’entr?e, objectif, stop-loss, risque principal. Sois direct.”)
...
f”? <b>Analyse IA:</b>\n{ai}\n\n”
f”<i>?? Pas un conseil financier</i>”)
...
prompt   = (f”3 meilleures cryptos d?tect?es parmi 500 :\n”
...
f”Laquelle choisir pour investir dans l’heure ? Prix d’entr?e et objectif concrets. 4 phrases max.”)
ai = ask_groq(prompt, 300)
return (f”? <b>MEILLEURE OPPORTUNIT? MAINTENANT</b>\n”
f”<i>{len(all_prices_cache)} cryptos analys?es</i>\n\n”
    ...
    try:
        ...
        if gp.get("code") == 1:
            ...
            checks = {
                ...
                "stealing_attack": "? Vol d?tect?",
                "blacklist_doubt": "?? Liste noire",
                "darkweb_transactions": "? Darkweb",
                "sanctioned": "? SANCTIONN?",
            }
...
prompt = (f"Analyse cette adresse crypto pour d?tecter une arnaque.\n"
          f"Adresse: {address}\nR?seau: {chain}\n"
          f"Signaux GoPlus: {goplus_result}\nFlags d?tect?s: {flags if flags else 'Aucun'}\n\n"
          f"Verdict: ARNAQUE / SUSPECT / L?GITIME. Niveau risque. Que faire. 4 phrases max.")
ai = ask_groq(prompt, 300)
flags_text = "\n".join(flags) if flags else "? Aucun signal d'arnaque"

return (f"?? <b>ANALYSE FRAUDE</b>\n"
        f"<code>{address[:20]}...{address[-6:]}</code>\n\n"
        f"? R?seau: <b>{chain}</b>\n"
        f"? GoPlus: <b>{goplus_result}</b>\n"
        f"{flags_text}\n\n"
        f"? <b>Verdict IA:</b>\n{ai}\n\n"
        f"? V?rifie aussi: etherscan.io")
```
...
if rsi >= RSI_SELL:
signal = “sell”; reasons = [f”? RSI ?lev? ({rsi})”]
if not signal:
continue
...
```
    ...
    msg  = (f"{icon} <b>{lbl} ? {d['name']} ({d['symbol']})</b> {e}\n\n"
            ...
            f"\n\n? {ai}\n\n"
            f"<i>? /analyse {d['symbol']} pour plus de d?tails\n"
            f"?? Pas un conseil financier</i>")
    send_telegram(msg)
...
def main():
print(”=” * 50)
print(”  CryptoMind Bot (Groq) ? D?marrage”)
print(”=” * 50)
send_telegram(”? <b>CryptoMind Bot ? D?MARR? ?</b>\n\n”
